...
3. **Execute an existing `plan.md`** — the user points at a finished plan or opts in to build one just produced. **Read `implementing.md` (next to this file) and follow it:** one fresh subagent per task, end-to-end verification against real expected behavior, a review between tasks, and a final whole-branch review, all in an isolated worktree.
